...
#        return False
    ...
    def _worker(
        ...
        output_queue: "multiprocessing.Queue[results.algorithm_output|results.error]"
    ):
        ...
        while not input_queue.empty():
            ...
            if subprocess_output.returncode == 0:
                ...
                result = results.error(input_obj.id, subprocess_output.stderr)
            output_queue.put(result)

    @staticmethod
    def _listener(
        input_queue: multiprocessing.Queue[results.algorithm_output | results.error | None],
        ...
        while True:
            result: results.algorithm_output | results.error | None = input_queue.get()
            if result is None:
                pipe.send(return_list)
            else:
                if isinstance(result, results.algorithm_output):
    ...
    def run(self) -> list[results.algorithm_output | results.error]:
        ...
        listener_q.put(None)
        listening.join()
        listener_output:list[results.algorithm_output|results.error] = PipeOut.recv() #Receive the list of outputs from the listener 
